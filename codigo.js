...
deshabilitar = (aux, tarea, num) => {
    ...
    output.innerHTML = "";
    output.innerHTML += `<div id="cb${aux}" ><li class="divTarea" id="cb${aux}"style="text-decoration:line-through;color: gray;"><input type="checkbox" id="flexCheckChecked " onclick="deshabilitar('${aux}',${tarea})" Checked disabled><p>${tarea}</p></li></div>`
...
MostrarHtml = () => {
    ...
    Tareas.forEach(n => {
        ...
        output.innerHTML += `<div id="cb${aux}" ><li style="${tachadoONo}"class="divTarea"><input type="checkbox" id="flexCheckChecked " onclick="deshabilitar('cb${aux}','${n.Nombre}',${aux})" ${checkeadoONo}><p>${n.Nombre}</p></li></div>`
        aux++;
    });
